...
-- Create function to automatically create offers when FBOs are added
CREATE OR REPLACE FUNCTION create_work_order_offer()
RETURNS TRIGGER AS $$
BEGIN
...
-- Create trigger to automatically create offers
CREATE TRIGGER auto_create_work_order_offer
  AFTER INSERT ON work_order_fbos
  FOR EACH ROW
  EXECUTE FUNCTION create_work_order_offer();